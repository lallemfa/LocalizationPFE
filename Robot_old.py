...
#        self.environment = environment
    ...
    def updateState(self, dt, waypoint):
        ...
        if (len(self.true_state_history) >= 2) and not (self.ins is None):
            ...
            pose, angles, velocities, angular_velocity, Gamma = self.integrateINS(dt, measures, self.previous_measure, self.estimated_state_history, view)
    ...
    def integrateINS(self, dt, measures, previous_measure, history, view, gammaINS):
...
# =============================================================================
        ...
        pose, Gamma = kalman(previous_state.pose, self.A, previous_state.gamma, dt*velocities, self.B, gammaINS, y, Gamma_obs, C)
...
# =============================================================================
    ...
    def computeTrajectoryWithNewINS(self, ins, dt, name = "No name"):
        ...
        if len(self.true_state_history) >= 2:
            ...
            for i in range(2, len(self.true_state_history)):
                ...
                pose, angles, velocities, angular_velocity, Gamma = self.integrateINS(dt, measures, previous_measure, new_estimated_history, view, gammaINS)
    ...
    def generateObservationForTBN(self, data):
        ...
        [Gx, Gy] = self.environment.gradient(view.T)

        view_img = self.environment.hsv_gradient(Gx, Gy, view.T)
        ...
        center, R, T = self.matcher.matchImages(view_img, self.full_map_img, False)
        
        if not((center is None) or (R is None) or (T is None)):
        
            X = self.environment.cloud[0, center[0], 0]
            Y = self.environment.cloud[center[1], 0, 1]
            ...
            Gamma_obs = 0.1*np.eye(2)
        
        if not((C is None) or (y is None) or (Gamma_obs is None)):
            print("Bouh")
